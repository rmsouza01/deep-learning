...
# Source code from: https://www.analyticsvidhya.com/blog/2021/08/how-to-code-your-resnet-from-scratch-in-tensorflow/

def resnet_identity_block(x, filter):
    # copy tensor to variable called x_skip
    x_skip = x
    # Layer 1
    x = tf.keras.layers.Conv2D(filter, (3,3), use_bias= False, padding = 'same')(x)
...
def resnet_convolutional_block(x, filter):
    # copy tensor to variable called x_skip
    x_skip = x
    # Layer 1
    x = tf.keras.layers.Conv2D(filter, (3,3), use_bias= False, padding = 'same', strides = (2,2))(x)
    x = tf.keras.layers.BatchNormalization(axis=3)(x)
    x = tf.keras.layers.Activation('relu')(x)
    # Layer 2
    x = tf.keras.layers.Conv2D(filter, (3,3), use_bias= False, padding = 'same')(x)
    ...
    x_skip = tf.keras.layers.Conv2D(filter, (1,1), use_bias= False, strides = (2,2))(x_skip)
    # Add Residue
    x = tf.keras.layers.Add()([x, x_skip])     
    x = tf.keras.layers.Activation('relu')(x)
    return x

def ResNet34(shape = (32, 32, 3), classes = 10,block_layers = [3, 4, 6, 3], filter_size = 64):

    # Step 1 (Setup Input Layer)
    x_input = tf.keras.layers.Input(shape)
    
    # Step 2 (Initial Conv layer along with maxPool)
    x = tf.keras.layers.Conv2D(filter_size, kernel_size=7, use_bias= False, strides=2, padding='same')(x_input)
    ...
    for i in range(len(block_layers)):
        if i == 0:
            ...
            for j in range(block_layers[i]):
                x = resnet_identity_block(x, filter_size)
        else:
            # One Residual/Convolutional Block followed by Identity blocks
            # The filter size will go on increasing by a factor of 2
            filter_size = filter_size*2
            x = resnet_convolutional_block(x, filter_size)
